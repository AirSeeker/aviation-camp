...
Surface weather observations are fundamental to all meteorological services. Observations are the basic
information upon which forecasts and warnings are made in support of a wide range of weather-sensitive
...
observation, sky condition is an evaluation of sensor data gathered during the 30-minute period ending at
...
plain language format in addition to the coded report. Table 24-1 provides an example of a decoded
...
A layer of clouds or obscurations aloft is present below 1,000 ft and no layer aloft
Sky Condition
...
Any other meteorological situation designated by the responsible agency of which,
Miscellaneous
...
the same as the International Air Transport Association (IATA) codes encountered by the general public
...
The visibility group is coded as the surface visibility in statute miles. A space is coded between whole
...
When blowing snow is observed with snow falling from clouds, both phenomena are reported
...
Other weather phenomenon types include well-developed dust/sand whirls (PO), sandstorms (SS), dust
...
Blowing sand in the vicinity.
Light rain and (light) snow, fog, haze.
...
+FC TSRAGR BR
Tornado, thunderstorm, (moderate) rain, hail, mist.
...
5,000-ft layer also exists above the lower layers (BKN). The sky condition group would be coded
...
Chapter 24, Observations
Scattered layer at 1,800 ft AGL, overcast layer (ceiling) of cumulonimbus
at 3,200 ft AGL.
...
Aircraft Mishap
24.
No SPECI Reports Taken NOTE: Additive data is primarily used by
25.
...
AO1 or AO2 is coded in all METARs/SPECIs from automated stations. Automated stations without a
...
When lightning is observed at a manual station, the frequency, type of lightning, and location are
reported. The contractions for the type and frequency of lightning are based on Table 24-7
...
Streaks of lightning that pass from a cloud to the air but do
Cloud-Air
CA
...
the pressure is rising or falling at a rate of at least 0.06 inch per hour and the pressure change totals 0.02
...
separated from each other by a solidus (/). For example, a snow depth increase of 2 inch in the past hour
...
T
Group indicator.
Indicates the following number is positive; a 1 would be used if the temperature was reported as
negative at the time of observation.
Temperature disseminated to the nearest tenth and read as 02.6.
Indicates the following number is negative; a 0 would be used if the number was reported as
positive at the time of observation.
Dewpoint disseminated to the nearest tenth and read as 01.5.
...
ft mountain. This means that the coverage begins at 10,000 ft AGL in that area. Any precipitation from
...
24.7.1 Description
Satellite is perhaps the single most important source of weather data worldwide, particularly over
...
approximately as it would appear when viewed with human eyes from space. At night, the blue colors
...
(approximately FL390). The actual numbers displayed on the water vapor images correspond to
...
the images. In general, regions displayed in shades of red are very dry in the upper atmosphere and may
...
battery-powered radio transmitter that sends the sensor measurements to a ground tracking antenna. Wind
...
for much improved situational awareness, seeing the weather picture in mountain passes. His work, along
...
FAA’s Aviation Weather Camera Network to produce visibility estimates (in statute miles) by analyzing
...
features in the landscape/scene and comparing them to a clear day reference. VEIA visibility estimates
allow users to quickly access visibility estimates at the departure location, along the route, and at the
...
outputs. It provides operators with visual and textual weather observations as guidance, for use with other
